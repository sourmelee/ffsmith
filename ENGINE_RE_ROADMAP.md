...
*Status 2026-06-08 (cont. 8): **New Game intro cinematics**. New Game now plays a 3-beat intro before the field (mirroring the real FFD: text crawl -> logo -> 'Prologue' card): beat 0 = the prologue crawl on deep blue (the real 'In an age long past... the Avalonian Empire...' string), beat 1 = the FF logo on cream, beat 2 = the 'Prologue' chapter card + flourish; advance with Z, then it loads the opening field. Toolkit 0.7.18 bakes `data/intro.bin` (prologue + chapter label) extracted BY CONTENT from msg0.msd (robust regardless of offsets). NOTE: msg{N}.msd strings are u16-BE length + NUL terminator (same as system_message.msd; lang order 0 ja, 1 en, ...; English text = m*12+2). The bake's `_read_msg_bank` already parses this correctly -- an earlier 'slot drift' worry was a throwaway u8-length parse mistake, NOT a field-dialogue bug. Verified `--intro 0|1|2` shots + `--menutest`. Notes: the bundle's logo is FINAL FANTASY **LEGENDS** (the Android port overlays its own FFD branding / a separate FFD logo asset we don't bake yet); the opening field is still the placeholder g0_p0_m500 -- the REAL opening is the **world map**, which needs the perspective renderer (3D tilt + Far background, deferred) to look like the game.*

*Fixes 2026-06-08: (1) New Game showed a **door** as the player + crashed (vector OOB) on opening the Debug menu -- New Game was leaving `dbgSprIdx_ = -1` (which the debug render indexed) and main auto-detected the first map event's sprite (a door). Fixed: bake each character's CHPK field-sprite id into chars.bin (toolkit 0.7.19; Sol=13), New Game uses the lead's CHPK (fallback 13), and the debug indices are clamped >=0. (2) MSVC C4244 int->uint8_t warning in the title-menu drawText -- cast. Build clean under MSVC + g++ -Wall -Wextra.*

*Status 2026-06-08 (cont. 9): **per-sprite field_anm geometry (object sprites)**. Jack flagged doors/crystals/chests rendering misaligned -- the engine applied the hardcoded 48x48 character grid to EVERY sprite, but each field_anm entry has its own frame size (chars 48x48/32x48, objects 16x16/20x24/32x16/32x32). Toolkit 0.7.20 bakes `data/spritegeo.bin` from field_anm: per sprite, the default (sub0 static) frame {x,y,w,h} + part-offset anchor (px,py) + isObject. Classifier: isObject = NO walk-cycle sub-animation (data-driven, not size -- correctly keeps short NPCs img50/53 as characters and flags the 18 cycle-less objects incl. img7 chest). Engine: drawSprite draws objects at `(lx+tile/2+px, ly+tile+py)` with src = the real frame; characters keep the 48x48 grid. Verified: img7 renders as a clean chest. Override hook: the bake merges `sprite_grid.json` ({img: {isObject,fx,fy,fw,fh,px,py}}) over the seed -- the manual-annotation path (Jack's preference). NEXT (Phase 2): author overrides in the toolkit's Animation tab (origin/anchor/frame controls + live preview -> writes sprite_grid.json), and generalize non-48 CHARACTER grids (Sol=32x48, img2=16x16) the same way.*
*Status 2026-06-10: **Event VM v2 — branching, flags/vars, appear conditions + the real New Game start.** RE'd the full script-execution model from `MoveEventScript`/`MoveScript`/`ScriptIf` (c:138937/139119/134322): blocks advance via a registry (`this+0xf438`: 0/1=jump, 3=fall-through, 4=end); **`ScriptIf` 0x3d is if-NOT-goto** (two `GetReference` operands + op + target block; jumps when the condition FAILS); 0x3f/0x40 jump to block indices; 0x3c = choice select (value→block pairs, cancel→next block); 0x41 takes five BE words with a var-indirection mask (all 15 real uses fully indirect). Flag banks (types 0-5, incl. paged 1/3/4 and the global GameClass bank 5) and variable banks (0-4 + system specials: msg bank, page, story state) decoded and implemented in `field/script_state.{h,cpp}`; `event_vm.cpp` is now a branching interpreter with choice pause/resume; **event appear conditions** (`CheckEventAppear`, 6 header slots: flag/flag/var/item/member/timer, FFV twin c:168498) gate NPC/trigger/solid/render. Verified on real data: m501's two stacked doors resolve by global flag5 bit 10 (clear→town 500, set→dark-world 1700, switchable via the new `--setflag`), and the Barbara NPC is correctly story-gated. Toolkit 0.7.24 bakes **FFM3** (+31 appear bytes/event — rebuild the engine before rebaking!) and `data/start.bin` from the newly decoded **boot_data §1 scenario table** (`LoadScenarioData`: 16 chapter records with start map/x/y + BeforeStory; `e3_param.dat` = E3 demo, not retail) — New Game now boots into the real opening map g0_p0_m0. Save = **FSAV v5** (+~4 KB script-state blob). Self-tests: `--vmtest` (11 checks), door-variant walk traces, v5 round-trip vs an independent Python parser; battlesim/menutest/itemtest/equiptest/leveltest all green. **Still open:** common events (boot 4/5 pool), auto-run boot-0 events on map load (the m0 prologue cutscene needs this), `GetReferenceEvent/Chara/Etc` stubs (logged), the page register (0xe474) semantics, choice-line text source, op 0x32 message-wait.*

*Status 2026-06-10 (cont.): **the opening plays — auto events, trigger rects, common-event pool** (toolkit 0.7.25, FFM4). RE'd how cutscenes start: `CheckRangeEvent` (c:113368) shows boots 6/7/8 are **rect triggers** (header[2..5] = x,y,w,h; boot 7 = "range-in always" fires every frame the player stands inside — including on map load), boots 4/5 run **whenever their appear conditions pass** (parallels), and the New Game spawn comes from a **map-header default position** (`+0xdc48..54`, consumed when `SetMapChange` layer = −1 — the scenario table's x/y fields are vestigial and never read). `0x66` was re-decoded as **CallEvent(id, block)** — the door "action 04" empiric was really a call to **common event 0x104**, the shared move-map routine living in **map 10000** (one global copy, named by `field_constant.dat` @0x8d): its preamble also corrected `0x41` to map/**layer**/x/y/dir. The engine now: fires on-load autos (`Field::enterMap`, with per-event + per-visit loop guards), pumps them sequentially between dialogues, holds warps until text is read, executes CallEvent inline (depth-capped, choice-aware), and treats `0x01 ScriptSentence` as dialogue. **Verified headlessly: the entire retail intro chain plays** — m0 (msg 53 "In an age long past…") → m101 → m100 (msg 50) → m101 → m200 (51/52) → m1 (the flyover hops, flat for now) → m300 (61, 62/60/63 "…sundered into light and darkness" + NameInput log-skip) — ending at the **scripted prologue battle** (`0x50 ScriptEncount`, log-skipped; post-battle dest = m150 via common 0x101). m501 doors now warp through the real 0x104 (both story variants), all self-tests green. **Next:** `ScriptEncount` + scripted-battle hand-off and post-battle resume (closes the intro into playable m150); common parallels (0x102/0x105/0x106…) not yet pumped; NameInput UI; fades/palette/NPC-move ops still log-skip.*
