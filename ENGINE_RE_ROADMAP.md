...
| M5 | **◑ Menu pages** — Item/Equip/Status show real baked data; **Item-use action wired** (inventory + counts + gil; Potion heals most-wounded member, decrements stack); equip-swap still pending | screenshots: 500-item list, Sol's Orichalcum, ATK 23; `--itemtest` Sol 1→70 |
...
| M7 | **◑ Save/load** — `save.dat` `FSAV` v2: map+pos+facing+sprite **+ persistent party (HP/MP) + inventory + gil**; menu Save / F5 / F9 / `--load`. Story-flags still pending | save m501@(6,11) → reload resumes; party(4)+Potion×5+500 gil round-trip byte-exact |
