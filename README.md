...
- **M3b (dialogue) — real on-screen text: ✅ done.** SetMessage now shows actual words in a bordered, word-wrapped, `\n`-aware dialogue box, from the decoded field message table. Field text lives in `system_message.msd` (6-language-interleaved records; **English = slot 1**; the engine's `GetMessageData` indexes the active language by msg_id). The toolkit bakes `text/messages.bin` + a font atlas (`text/font.tex`/`.meta`); the engine renders it. Verified via `--fieldshot --open-msg`: m501's NPC (msg 170) shows “Hero: Barbara!” and m500's cutscene (msg 150) wraps “Vata: Hoho, this is perfect…” across lines.
- **Next:** per-chapter message-bank selection (high msg ids like m501's 847 use a shared bank), range-trigger extents, common-event (boot 4/5) + auto (boot 0) events, then **M4** game state machine.
