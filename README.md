...
- **M3b (sprites) — real field sprites + facing: ✅ done.** Player + NPCs render as actual `fldchr` sprites (feet-aligned, transparent), now **facing the right way** — per-facing standing frame decoded from `field_anm.dat` (48×48 grid, origin (1,1), pitch 50; DOWN=(51,1), UP=(1,51), LEFT=(1,1), RIGHT=(101,1)). Verified in-engine (`--fieldshot --face`). Walk-cycle animation (sheets are part/keyframe-composed via `field_anm` sub[5]) is a later refinement.
