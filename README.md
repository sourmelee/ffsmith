...
- **Persistent party / inventory — ✅.** A mutable `GameState` (the 4-member party with **current HP/MP**, an **inventory** of item+count, and **gil**) is initialised from the baked `chars.bin` and now survives across battles and saves. Battle builds combatants from it and **writes HP/MP back** when the fight ends (verified: Aigis leaves a fight at 198/199, not refilled). The **Item** menu lists the **owned inventory** (counts + gil) and **using a Potion** heals the most-wounded member and decrements the stack (`[A] Use`; verified Sol 1→70 HP, Potion ×5→×4). Status shows live HP/MP. All three are serialized into `save.dat` (`FSAV` v2) and restored on load (round-trip verified byte-exact). Headless self-test: `--itemtest`.
- **Next:** equipment swapping from inventory, EXP/gil battle rewards + level-ups, then encounter tables.
